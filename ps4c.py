...
def decrypt_message_try_pads(ciphertext, pads):
    ...
    metric = lambda p: len([w for w in ciphertext.decrypt_message(p).get_text().split(" ") if is_word(WORD_LIST,w)])
    best_pad = max(reversed(pads),key=metric)
...
def decode_story():
    ...
    '''
    return decrypt_message_try_pads(ps4b.EncryptedMessage(get_story_string()),get_story_pads())
